...
#define ZEND_MM_UNPOISON_CHUNK(_ptr) ZEND_MM_UNPOISON((_ptr), sizeof(zend_mm_chunk));

#else

#define ZEND_MM_POISON
#define ZEND_MM_UNPOISON
...
{
	ZEND_MM_UNPOISON_CHUNK(chunk);
	ZEND_MM_UNPOISON_CHUNK(chunk->next);
	ZEND_MM_UNPOISON_CHUNK(chunk->prev);
	...
	ZEND_MM_POISON_CHUNK(chunk->next, heap);
	ZEND_MM_POISON_CHUNK(chunk->prev, heap);
...
static zend_always_inline void zend_mm_set_next_free_slot(const char *from, zend_mm_heap *heap, uint32_t bin_num, zend_mm_free_slot *slot, zend_mm_free_slot *next)
{
	...
	ZEND_MM_UNPOISON(slot, 8);
	ZEND_MM_UNPOISON(&ZEND_MM_FREE_SLOT_PTR_SHADOW(slot, bin_num), 8);

	slot->next_free_slot = next;
	ZEND_MM_FREE_SLOT_PTR_SHADOW(slot, bin_num) = zend_mm_encode_free_slot(heap, next);

	ZEND_MM_POISON(slot, 8);
	ZEND_MM_POISON(&ZEND_MM_FREE_SLOT_PTR_SHADOW(slot, bin_num), 8);
}
...
ZEND_API void zend_mm_shutdown(zend_mm_heap *heap, bool full, bool silent)
{
	...
	ZEND_MM_UNPOISON_CHUNK(heap->main_chunk);
...
{
#if ZEND_MM_CUSTOM
	ZEND_MM_UNPOISON(AG(mm_heap), sizeof(zend_mm_heap));
	bool ret = !AG(mm_heap)->use_custom_heap;
	ZEND_MM_POISON(AG(mm_heap), sizeof(zend_mm_heap));
	return ret;
#else
	return 1;
#endif
		...
		do {
			if (ptr >= (void*)chunk
			 && ptr < (void*)((char*)chunk + ZEND_MM_CHUNK_SIZE)) {
				ZEND_MM_POISON_CHUNK(chunk, AG(mm_heap));
				ZEND_MM_POISON(AG(mm_heap), sizeof(zend_mm_heap));
				return 1;
			}
			next = chunk->next;
