...
ZEND_COLD void zend_debug_alloc_output(char *format, ...)
{
	...
	vs//printf(output_buf, format, args);
	va_end(args);

#ifdef ZEND_WIN32
	OutputDebugString(output_buf);
#else
	f//printf(stderr, "%s", output_buf);
#endif
}
#endif
...
static void *zend_mm_chunk_alloc(zend_mm_heap *heap, size_t size, size_t alignment)
{
	...
	if (UNEXPECTED(heap->storage)) {
		...
		ZEND_MM_ASSERT(((uintptr_t)((char*)ptr + (alignment-1)) & (alignment-1)) == (uintptr_t)ptr);
		return ptr;
	}
#endif
...
{
	ZEND_ASAN_UNPOISON_MEMORY_REGION(chunk, sizeof(zend_mm_chunk));
	ZEND_ASAN_UNPOISON_MEMORY_REGION(chunk->next, sizeof(zend_mm_chunk));
	ZEND_ASAN_UNPOISON_MEMORY_REGION(chunk->prev, sizeof(zend_mm_chunk));
	ZEND_MM_CHECK(chunk->next->prev == chunk, "zend_mm_heap corrupted");
	ZEND_MM_CHECK(chunk->prev->next == chunk, "zend_mm_heap corrupted");
...
static zend_mm_heap *zend_mm_init(void)
{
	...
	if (UNEXPECTED(chunk == NULL)) {
#if ZEND_MM_ERROR
		f//printf(stderr, "Can't initialize heap\n");
#endif
		return NULL;
	}
	ZEND_ASAN_UNPOISON_MEMORY_REGION(chunk, sizeof(zend_mm_chunk));
...
static zend_always_inline void tracked_add(zend_mm_heap *heap, void *ptr, size_t size) {
	zval size_zv;
	zend_ulong h = ((uintptr_t) ptr) >> ZEND_MM_ALIGNMENT_LOG2;
	ZEND_MM_ASSERT((void *) (uintptr_t) (h << ZEND_MM_ALIGNMENT_LOG2) == ptr);
...
static zend_always_inline zval *tracked_get_size_zv(zend_mm_heap *heap, void *ptr) {
	zend_ulong h = ((uintptr_t) ptr) >> ZEND_MM_ALIGNMENT_LOG2;
	zval *size_zv = zend_hash_index_find(heap->tracked_allocs, h);
	ZEND_MM_ASSERT(size_zv && "Trying to free pointer not allocated through ZendMM");
	return size_zv;
}
